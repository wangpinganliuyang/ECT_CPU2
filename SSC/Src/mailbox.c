...
void MBX_MailboxWriteInd(TMBX MBXMEM *pMbx)
{
    ...
    UINT16 MbxLen = pMbx->MbxHeader.Length;
    ...
    if(MbxLen > MAX_MBX_SIZE)
    {
        ...
        pMbx->Data[0]                        = MBXSERVICE_MBXERRORCMD;
        pMbx->Data[1]                        = MBXERR_INVALIDSIZE;
...
//             Mailbox error response: type 0 (mailbox service protocol)
            ...
            pMbx->MbxHeader.Flags        &= ~(MBX_MASK_TYPE);
            pMbx->Data[0]                        =MBXSERVICE_MBXERRORCMD;
            pMbx->Data[1]                        = MBXERR_NOMOREMEMORY;
            MBX_MailboxSendReq(pMbx, 0);
...
//                MEMCPY((UINT16 *)&u16dummy,(((UINT8 *)pMbx) + LastDataAddress),(2 - BytesLeft));//修改
            ...
            ESC_writeWordNonISR(u16dummy,(u16EscAddrSendMbx + u16SendMbxSize - 2));
        }
        }
