...
static UINT8 SdoDownloadSegmentInd( TDOWNLOADSDOSEGREQMBX MBXMEM * pSdoInd )
{
    ...
    if ( (pSdoInd->SdoHeader.SegHeader & SEGHEADER_TOGGLE) == bSdoSegLastToggle )//修改
    {
    ...
    else
    {
        ...
        bSdoSegLastToggle = pSdoInd->SdoHeader.SegHeader & SEGHEADER_TOGGLE;//修改
        ...
        if ( pSdoSegData )
        {
            ...
            if ( pSdoInd->SdoHeader.SegHeader & SEGHEADER_NOMOREFOLLOWS )//修改
            {
...
//                MBXMEMCPY( ((UINT8*)pSdoSegData) + (nSdoSegBytesToHandle), pSdoInd->SdoHeader.Data, bytesToSave ); //修改
            	//内存对齐问题，修改
            	//本段为SDO分段下载，分段下载的邮箱报文数据先储存到psdosegdata中，带分段下载完成后，在整体更新对象字典
            	//由nSdoSegBytesToHandle已经操作的字节数，若奇数，首端需要整合，若偶数，末端整合
            	/*if(nSdoSegBytesToHandle & 0x1)//奇数
            	{
            		((UINT16 *)pSdoSegData)[((nSdoSegBytesToHandle-1)>>1)] = ((UINT16 *)pSdoSegData)[((nSdoSegBytesToHandle-1)>>1)]  + ((pSdoInd->SdoHeader.Data[0]) & 0xFF00);
            		if(bytesToSave>1)
            		{
            			for(bytesToSavei=0;bytesToSavei<((bytesToSave-1)>>1);bytesToSavei++)
            			{
            				((UINT16 *)pSdoSegData)[((nSdoSegBytesToHandle+1)>>1) + bytesToSavei]= pSdoInd->SdoHeader.Data[bytesToSavei+1];
            			}
            		}
            	}
            	else//偶数
            	{
            		for(bytesToSavei=0;bytesToSavei<((bytesToSave-1)>>1);bytesToSavei++)
            		{
            		    Data0 =  (pSdoInd->SdoHeader.Data[bytesToSavei])>>8;
            			Data1 = (pSdoInd->SdoHeader.Data[bytesToSavei+1]) & 0x00FF;
            			((UINT16 *)pSdoSegData)[((nSdoSegBytesToHandle)>>1) + bytesToSavei] = Data0 + (Data1<<8);
            		}
            		((UINT16 *)pSdoSegData)[((nSdoSegBytesToHandle)>>1)+((bytesToSave-1)>>1)] =  (pSdoInd->SdoHeader.Data[((bytesToSave-1)>>1)])>>8;
            	}*/
            	//修改完成
    ...
    if ( abort == 0)
    {
        ...
        {
           pSdoInd->SdoHeader.SegHeader        = SDOSERVICE_DOWNLOADSEGMENTRES|SEGHEADER_TOGGLE;//修改
//           pSdoInd->SdoHeader.Data[0]        = ((pSdoInd->SdoHeader.Data[0]) & 0xFF00) + SDOSERVICE_DOWNLOADSEGMENTRES|SEGHEADER_TOGGLE;//修改
        }
        else
           pSdoInd->SdoHeader.SegHeader        = SDOSERVICE_DOWNLOADSEGMENTRES;//修改
//           pSdoInd->SdoHeader.Data[0]        = ((pSdoInd->SdoHeader.Data[0]) & 0xFF00) + SDOSERVICE_DOWNLOADSEGMENTRES;//修改
...
//   if ( (SegHeader0 & SEGHEADER_TOGGLE) == bSdoSegLastToggle )//修改
    {
        /* toggle bit has not toggled... */
        abort = ABORTIDX_TOGGLE_BIT_NOT_CHANGED;
    }
...
//        bSdoSegLastToggle = SegHeader0 & SEGHEADER_TOGGLE;//修改
    ...
    if ( abort == 0 )
    {
        ...
        if ( command == SDOSERVICE_INITIATEUPLOADREQ ) /**< \brief SDO upload request*/
        {
            ...
            if ( (objLength <= 4) && (objLength > 0) )
            {
                ...
                pSdoRes->SdoHeader.Sdo[SDOHEADER_COMMANDOFFSET]    |=     0x0001     |
                                                                                        0x0002        |
                                                                                        completeAccess |
...
//                                                                                                        SDOSERVICE_INITIATEUPLOADRES;
            ...
            else
            {
                ...
                else
                {
                   ...
                   ((TINITSDOUPLOADNORMRESMBX MBXMEM *) pSdoRes)->CompleteSize[0] = (UINT16)objLength;
                   ((TINITSDOUPLOADNORMRESMBX MBXMEM *) pSdoRes)->CompleteSize[1] = (UINT16)(objLength>>16);
...
//            pSdoRes->SdoHeader.Sdo[0]    |= SDOSERVICE_INITIATEDOWNLOADRES;
    ...
    else
    {
        ...
        pSdoRes->SdoHeader.Sdo[SDOHEADER_COMMANDOFFSET]    = 0x0080;//SDOSERVICE_ABORTTRANSFER;
...
//        index += Sdo1 >> SDOHEADER_INDEXLOSHIFT;
...
/*        if ((index >=0x2000) & (index <= 0x3000))
        {
        	...
        	UINT32 *pData = NULL;
        	{
        		...
        		if(command == SDOSERVICE_INITIATEUPLOADREQ)*< \brief SDO upload request
        		{
        			...
        			Shared_Ram_cpu2_WR_Upload(index,subindex);
        		}
        		else
        		{
        	...
        	if( command == SDOSERVICE_INITIATEDOWNLOADREQ )*< \brief SDO download request
        	{
        		...
        		if(bTransferType)
        		{
                    ...
                    //非周期参数写操作
                    v2->GDCIndexValue = pData[0];
                    v2->GDCSub = (index - 0x2000) + (((UINT32)subindex)<< 16);
                    v2->Flags.all |= 0x0010;
        		}
        	}
            if ( abort == 0 )
...
//            	    do{
//
//            	    }while(CPU1_CPU2.Flags.bits.UnCycleOrderR != 2);
//            	    if(CPU1_CPU2.Flags.bits.UnCycleOrderR ==2)
//            		{
//            			pData[0] = CPU1_CPU2.GDCIndexValue;
//            			CPU2_CPU1.Flags.bits.UnCycleOrderR = 0;
//             		}
                    ...
                    if ( abort == ABORTIDX_WORKING )
                    {
                        ...
                        pSdoResStored->SdoHeader.Sdo[SDOHEADER_COMMANDOFFSET] &= ~(0x00FF);//~SDOHEADER_COMMANDMASK;
        ...
        if ( pObjEntry )
        {
            ...
            if ( command == SDOSERVICE_INITIATEDOWNLOADREQ )/**< \brief SDO download request*/
            {
                ...
                else
                {
                    ...
                    UINT32 downloadSize = ((UINT32)(((TINITSDODOWNLOADNORMREQMBX MBXMEM *) pSdoInd)->CompleteSize[1])<<16) + (((TINITSDODOWNLOADNORMREQMBX MBXMEM *) pSdoInd)->CompleteSize[0]);
                ...
                else
                {
                    ...
                    if ( command == SDOSERVICE_INITIATEUPLOADREQ )/**< \brief SDO upload request*/
                    {
                        ...
                        if ( abort == ABORTIDX_WORKING )
                        {
                            ...
                            u32StoreDataSize = objLength;
                            pStoreData = pData;
                            pSdoPendFunc = OBJ_Read;//pObjEntry->Read;
...
//                            pSdoResStored->SdoHeader.Sdo[0] &= ~(0x00FF);
                            //修改，其它情况1000011
                            pSdoResStored->SdoHeader.Sdo[SDOHEADER_COMMANDOFFSET]    |= 0x0043;//(sdoHeader & (SDOHEADER_COMPLETEACCESS | SDOHEADER_COMMAND));
//                            pSdoResStored->SdoHeader.Sdo[0]    |= ((sdoHeader & (SDOHEADER_COMPLETEACCESS | SDOHEADER_COMMAND)));
                            return 0;
                        }
                    }
...
void SDOS_SdoRes(UINT8 abort, UINT32 objLength, UINT16 MBXMEM *pData)
{
    ...
    if (bSdoInWork)
    {
        ...
        UINT8 command = pSdoResStored->SdoHeader.Sdo[SDOHEADER_COMMANDOFFSET] & 0xE0;//SDOHEADER_COMMAND;
//        UINT8 command = ((pSdoResStored->SdoHeader.Sdo[0])) & SDOHEADER_COMMAND;//修改
        UINT8 completeAccess = pSdoResStored->SdoHeader.Sdo[SDOHEADER_COMMANDOFFSET] & 0x10;//SDOHEADER_COMPLETEACCESS;
...
UINT8 SDOS_SdoInfoInd( TSDOINFORMATION MBXMEM *pSdoInfoInd )
{
    ...
    case SDOINFOSERVICE_OBJDICTIONARYLIST_Q://get od list请求
        ...
        if ( (pSdoInfoInd->SdoHeader.Data.List.ListType) <= INFO_LIST_TYPE_MAX )
        {
            UINT16 size = 0;
            /* the variable listType contains the requested listType */
            UINT8 listType = (UINT8) (pSdoInfoInd->SdoHeader.Data.List.ListType);
 ...
 //           MBXMEMCPY(aSdoInfoHeader, pSdoInfoInd, SDO_INFO_HEADER_BYTE_SIZE);
            ...
            if ( listType-- == 0 )//在5个不同的列表中得到对象的数量
            {
                ...
                for (i = 0; i < INFO_LIST_TYPE_MAX; i++)
                {
                    ...
                    ((UINT16 MBXMEM *) &pSdoInfoInd->CoeHeader)[(SIZEOF_SDOINFOLISTSTRUCT>>1)+i] = n;
                }
