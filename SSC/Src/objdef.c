...
V4.04 SDO 1: The SDO interface was changed in that way that a SDO response<br>
             could be sent by the application to a later time. In that case<br>
                 ...
                 . To send the SDO response the new function SDOS_SdoRes<br>
...
char          aSubindexDesc[13]= { "SubIndex 000" };
////修改、便于统一
/*void objdef_Init(void)
{
    TOBJ10F1 sErrorSettings = {
                      2,
                      0x01,
...
/*ECATCHANGE_START(V5.11) OBJ1*/
    ...
    if(subindex > 0)
    {
    	...
    	if((Index == 0x1018) | (Index == 0x10F1) | (Index == 0x1601) | (Index == 0x1602)
    			| (Index == 0x1A01) | (Index == 0x1A02)
...
//        if(count >= 500)
        {
            pData[0] = (v1->GDCIndexValue_Up & 0x0000FFFF);
            pData[1] = (v1->GDCIndexValue_Up >> 16);
            v2->Flags.all &= 0xFFF9;
            count = 0;
            return 0;
        }
                ...
                if ( ((UINT8)((pEntry->ObjAccess & ACCESS_WRITE) >> 2)) >= (nAlStatus & STATE_MASK) )
                {
                      ...
                      || (bCompleteAccess && i >= subindex) )       /* complete access and entry should be read */
                    {
                        ...
                        if (i == 0 && objCode != OBJCODE_VAR)
                        {
                            ...
                            UINT8 NewSubindex0 = (UINT8) pData[0];
