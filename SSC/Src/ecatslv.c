...
void ResetALEventMask(UINT16 intMask)
{
    ...
    ESC_writeWordNonISR(mask, ESC_AL_EVENTMASK_OFFSET);
}
...
TSYNCMAN ESCMEM * GetSyncMan( UINT8 channel )
{
	...
	if(channel == 0)
	{
	    SyncManInfo.PhysicalStartAddress = 0x1000;
	    SyncManInfo.Length = 0x0080;
	    SyncManInfo.Settings[0] = ESC_readWordNonISR(0x0804);
	    SyncManInfo.Settings[1] = 0x0001;
	}
	else if (channel == 1)
	{
        SyncManInfo.PhysicalStartAddress = 0x1400;
        SyncManInfo.Length = 0x0080;
        SyncManInfo.Settings[0] = ESC_readWordNonISR(0x080C);
        SyncManInfo.Settings[1] = 0x0001;
	}
	else if (channel == 2)
	{
	...
	else if(channel == 3)
	{
        ...
        else if(sTxPDOassign.aEntries[0] == 0x1A02)
        {
            SyncManInfo.Length = 0x000a;
        }
        else
        {
            SyncManInfo.Length = 0x0006;
        }
        SyncManInfo.Settings[0] = ESC_readWordNonISR(0x081C);
        SyncManInfo.Settings[1] = 0x0001;
	}
	else
	{
        SyncManInfo.PhysicalStartAddress = 0;
        SyncManInfo.Length = 0;
        SyncManInfo.Settings[0] = 0;
        SyncManInfo.Settings[1] = 0;
	}
...
/*ECATCHANGE_END(V5.11) HW1*/
    ...
    */
    ...
    dcControl = ESC_readWordNonISR(ESC_DC_UNIT_CONTROL_OFFSET);//DC模式下为0x0300,Free模式下为0X0
    ...
    cycleTimeSync0 = (((UINT32)cycleTimeSync02)<<16) + cycleTimeSync01;//DC模式下为2000000
...
//    cycleTimeSync12 = ESC_readWordNonISR((ESC_DC_SYNC1_CYCLETIME_OFFSET+2));
    cycleTimeSync1 = 0;//(((UINT32)cycleTimeSync12)<<16) + cycleTimeSync11;
...
/*ECATCHANGE_END(V5.11) ECAT4*/
    ...
    */
    ...
    wd = 1000;//ESC_readWordNonISR(ESC_PD_WD_TIME);
/*ECATCHANGE_END(V5.11) ECAT4*/

    if (nPdOutputSize > 0 &&  wd != 0 )
    {
    /*get watchdog divider (register 0x400)*/
    	wdiv = 2498;//ESC_readWordNonISR(ESC_WD_DIVIDER_OFFSET);
        if ( wdiv != 0 )
        {
...
//            else
//            {
                Sync0WdValue = 4;//(UINT16)(Sync0Cycle*2)/10;
//            }
//        }
...
void SetALStatus(UINT8 alStatus, UINT16 alStatusCode)
{
    ...
    {
    	ESC_writeWordNonISR(Value,ESC_AL_STATUS_CODE_OFFSET);
    }

    Value = nAlStatus;
    ESC_writeWordNonISR(Value,ESC_AL_STATUS_OFFSET);
...
/*ECATCHANGE_END(V5.12) ECAT7*/
    ...
    UINT16 TmpVar = 0;
    TmpVar = (((UINT16)u8EcatRunLed) | (((UINT16)u8EcatErrorLed)<<8));
    ESC_writeWordNonISR(TmpVar,ESC_RUN_LED_OVERRIDE);
...
//                            StatusCode = ALSTATUSCODE_SMWATCHDOG;//修改
                        }
                        else
                        {
...
void CheckIfEcatError(void)
{
   ...
   if (EcatWdValue != 0)
   {
      ...
      WdStatusOK = ESC_readWordNonISR(ESC_PD_WD_STATE);//DC模式下为0
...
void ECAT_Init(void)
{
    ...
    TmpVar = 0x0808;//ESC_readWordNonISR(ESC_COMM_INFO_OFFSET);

    nMaxSyncMan = (UINT8) ((TmpVar & ESC_SM_CHANNELS_MASK)>> ESC_SM_CHANNELS_SHIFT);

    TmpVar = 0x0f08;//ESC_readWordNonISR(ESC_DPRAM_SIZE_OFFSET);
    ...
    */
    if ( bMbxRunning )
    {
        ...
        if ( ALEventReg & (MAILBOX_READ_EVENT) )
        {
            ...
            u16dummy = 0;
            ESC_writeWordNonISR(u16dummy,u16EscAddrSendMbx);
            ...
            ||( !(sm1Activate & SM_SETTING_REPAET_REQ_MASK) && bMbxRepeatToggle ))
        {
            ...
            ESC_writeWordNonISR(sm1Activate,(ESC_SYNCMAN_ACTIVE_OFFSET + SIZEOF_SM_REGISTER));
        }
        EINT;
